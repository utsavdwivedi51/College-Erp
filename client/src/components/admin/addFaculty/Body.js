...
const Body = () => {
  ...
  const createdFacultyCredentials = useSelector(
    (state) => state.admin.createdFacultyCredentials
  );
  ...
  const handleSubmit = (e) => {
    ...
    setError({});
    setLoading(true);
    dispatch({ type: SET_CREATED_FACULTY_CREDENTIALS, payload: null });
      ...
      <div className="space-y-5">
        ...
        <div className=" mr-10 bg-white flex flex-col rounded-xl ">
          <form className={classes.adminForm0} onSubmit={handleSubmit}>
            ...
            {createdFacultyCredentials?.username && (
              <div className="border border-green-200 bg-green-50 text-green-900 rounded-lg px-4 py-3 mt-4 w-full max-w-2xl">
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1 text-sm">
                    <p className="font-semibold text-base">
                      Faculty credentials generated successfully
                    </p>
                    {createdFacultyCredentials?.name && (
                      <p>
                    ...
                    <p>
                      <span className="font-semibold">Initial Password:</span>{" "}
                      {createdFacultyCredentials.initialPassword}
                    </p>
                    <p className="text-xs text-green-800">
                      Ask faculty to login once and change password immediately.
                    </p>
                  </div>
                  <button
                    type="button"
                    className="text-xs bg-green-100 hover:bg-green-200 rounded px-2 py-1"
                    onClick={() =>
                      dispatch({
                        type: SET_CREATED_FACULTY_CREDENTIALS,
                        payload: null,
                      })
                    }>
                    Dismiss
